...
namespace Nu
{
    ...
    struct EventListener
    {
        ...
        NU_INLINE EventListener(CallbackFn&& callback, uint32_t listnrid):
    ...
    struct EventDispatcher
    {
        ...
        NU_INLINE void AttachCallback(Callback&& callback, uint32_t listnrid)
        {
        ...
        template<typename Event>
        NU_INLINE void DetachCallback(uint32_t listnrid)
        {
        ...
        NU_INLINE void EraseListener(uint32_t listnrid)
        {
    ...
    private:
        template<typename Event>
        NU_INLINE EventRegistry<Event>* CastRegistry(void* p)
        {
            return static_cast<EventRegistry<Event>*>(p);
        }

        template<typename Event>
        NU_INLINE EventRegistry<Event>* GetRegistry()
        {
