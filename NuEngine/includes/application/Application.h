...
namespace Nu
{
    ...
    private:
        NU_INLINE void RegisterEventCallbacks()
        {
        ...
        NU_INLINE void ComputeFrameDeltaTime()
        {
        ...
        NU_INLINE void RunPhysicsSimulation()
        {
            // compute physx
        ...
        NU_INLINE void DestroyPhysicsActors()
        {
            EnttView<Entity, RigidBodyComponent>([this] (auto entity, auto& comp) 
            { 
        ...
        NU_INLINE void CreateSkyboxEnvMaps()
        {
        ...
        NU_INLINE void RenderSceneDepthMap()
        {        
        ...
        NU_INLINE void CreatePhysicsActors()
        {
            EnttView<Entity, RigidBodyComponent>([this] (auto entity, auto& comp) 
            { 
        ...
        NU_INLINE void CreateSceneEntities()
        {
            // load models
            ...
            // create sphere entity rigid body
            for(uint32_t i = 0; i < 100; i++)
            {
                ...
                ts.Translate = glm::vec3(3.0f, 5.0f * i, -10.0f);
                ts.Scale *= 5.0f; 
            }

            // create cube entity rigid body
            for(uint32_t i = 0; i < 100; i++)
            {
                ...
                tc.Translate = glm::vec3(-3.0f, 10.0f *i, -15.0f);
                tc.Rotation.x = i * 10.0f;
                tc.Scale *= 5.0f;
            }
        }
               
        // renders scene to the frame buffer
        NU_INLINE void RenderSceneToFBO()
        {
            // start new frame
        ...
        NU_INLINE void UpdateAppLayers()
        {
