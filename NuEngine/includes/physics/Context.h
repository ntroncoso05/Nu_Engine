...
namespace Nu
{
    struct PhysicsContext
    {
        NU_INLINE PhysicsContext() 
        {
            ...
            if (!m_Physics) 
            {
                NU_ERROR("Error initializing PhysX m_Physics");
            ...
            if (!m_Scene) 
            {
                NU_ERROR("Error creating PhysX m_Scene");
                m_Physics->release();
                m_Foundation->release();
                return;
            }   
        ...
        NU_INLINE void AddRigidBody(Entity& entity)
        {                   
            ...
            if(entity.template Has<ColliderComponent>())
            {
                ...
                {
                    NU_ERROR("Error creating collider invalid type provided");
                    return;
                }
            ...
            if (!body.Actor) 
            {
                NU_ERROR("Error creating dynamic actor");               
                return;
            }
        ...
        NU_INLINE void Simulate(uint32_t step, float dt)
        {
        ...
        NU_INLINE void SetEventCallback(PxCallbackFunction&& callback)
        {
            m_EventCallback.m_Callback = callback;
        }

        NU_INLINE PxConvexMeshGeometry CookMesh(const MeshData<ShadedVertex>& data) 
        {          
            // px vertex container
