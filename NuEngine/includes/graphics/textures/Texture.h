...
{  
    struct Texture2D 
    {
        ...
        NU_INLINE bool Load(const std::string& path, bool isHDR = false, bool flipY = true) 
        {
            ...
            if(pixels == nullptr) 
            { 
                NU_ERROR("failed to load texture!");
                return false; 
            }
        ...
        NU_INLINE int32_t Width() const { return m_Width; }
        NU_INLINE uint32_t ID() const { return m_ID; }

        NU_INLINE void Bind() { glBindTexture(GL_TEXTURE_2D, m_ID); }
        NU_INLINE void Unbind() { glBindTexture(GL_TEXTURE_2D, 0); }

        NU_INLINE void Use(uint32_t uniform, int32_t unit) 
        { 
