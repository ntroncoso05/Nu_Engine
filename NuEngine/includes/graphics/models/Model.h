...
namespace Nu
{
	...
	struct Model 
	{
        NU_INLINE virtual bool HasJoint() { return false; }
		NU_INLINE virtual void Load(const std::string&) {}
		NU_INLINE virtual void Draw(uint32_t mode) {}
	};	

	// static model
    struct StaticModel : Model
    {
        NU_INLINE StaticModel() = default;

        NU_INLINE StaticModel(const std::string& path)
        {
            Load(path);
        }

        NU_INLINE void Load(const std::string& path) override final
        {
            uint32_t flags = aiProcess_Triangulate | aiProcess_GenSmoothNormals | aiProcess_CalcTangentSpace |
                             ...
                             aiProcess_ImproveCacheLocality | aiProcess_FixInfacingNormals | aiProcess_GenUVCoords | aiProcess_FlipUVs;
            ...
            {
                NU_ERROR("failed to load model: {}", importer.GetErrorString());
                return;
            }
        ...
        NU_INLINE void Draw(uint32_t mode) override final
        {
    ...
    private:
        NU_INLINE void ParseNode(const aiScene* ai_scene, aiNode* ai_node)
        {
        ...
        NU_INLINE void ParseMesh(aiMesh* ai_mesh)
        {
	...
	struct SkeletalModel : Model
	{
		...
		NU_INLINE SkeletalModel(const std::string& path)
		{
			Load(path);
		}

		NU_INLINE SkeletalModel() = default;
        
        NU_INLINE bool HasJoint() override final { return m_JointCount; }

		NU_INLINE void Load(const std::string& path) override final
        {
			uint32_t flags = aiProcess_Triangulate | aiProcess_GenSmoothNormals | aiProcess_CalcTangentSpace |
				aiProcess_OptimizeMeshes | aiProcess_OptimizeGraph | aiProcess_ValidateDataStructure |
				aiProcess_ImproveCacheLocality | aiProcess_FixInfacingNormals | aiProcess_SortByPType |
                aiProcess_JoinIdenticalVertices | aiProcess_FlipUVs | aiProcess_GenUVCoords | aiProcess_LimitBoneWeights; 
		...
		NU_INLINE auto GetAnimator()
		{
			return m_Animator;
		}

	private:
		NU_INLINE void ParseNode(const aiScene* ai_scene, aiNode* ai_node, JointMap& jointMap) 
        {
			for (uint32_t i = 0; i < ai_node->mNumMeshes; i++) 
            {
		...
		NU_INLINE void SetVertexJoint(SkeletalVertex& vertex, int32_t id, float weight) 
		{
		...
		NU_INLINE void ParseHierarchy(aiNode* ai_node, Joint& joint, JointMap& jointMap)
		{
		...
		NU_INLINE void ParseAnimations(const aiScene* ai_scene, JointMap& jointMap) 
        {
		...
		NU_INLINE void ParseMesh(const aiMesh* ai_mesh, JointMap& jointMap) 
        {
			// mesh data
