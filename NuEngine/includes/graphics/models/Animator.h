...
namespace Nu
{
    struct Animator 
    {
        NU_INLINE auto& Animate(float deltaTime)
        {
    ...
    private:                   
        NU_INLINE glm::mat4 Interpolate(const KeyFrame& prev, const KeyFrame& next, float progression) 
        {
        ...
        NU_INLINE void GetPreviousAndNextFrames(Joint& joint, KeyFrame& prev, KeyFrame& next) 
        {          
        ...
        NU_INLINE void UpdateJoints(Joint& joint, const glm::mat4& parentTransform) 
        {
            KeyFrame prev, next;
