...
namespace Nu
{
    ...
    { 
        NU_INLINE BloomShader(const std::string& path, int32_t width, int32_t height): 
        ...
        NU_INLINE void Compute(uint32_t brightnessMap, uint32_t stepCount)
        {
            // bind shader program
        ...
        NU_INLINE uint32_t GetMap()
        {
            return m_PingPongMaps[0];
        }

        NU_INLINE ~BloomShader()
        {
