...
namespace Nu
{
    ...
    { 
        NU_INLINE PrefilteredShader(const std::string& path): Shader(path) 
        {
        ...
        NU_INLINE uint32_t Generate(uint32_t skyCubeMap, SkyboxMesh& mesh, int32_t size) 
        {            
            // view matrices
            glm::mat4 views[] = 
            {
            ...
            for (uint32_t mip = 0; mip < nbrMipLevels; ++mip) 
            {
                ...
                glBindRenderbuffer(GL_RENDERBUFFER, RBO); // ? repeated line 64
