...
namespace Nu
{
    ...
    {
        NU_INLINE SkyMapShader(const std::string& path) : Shader(path)
        {
        ...
        NU_INLINE uint32_t Generate(Texture& texture, SkyboxMesh& mesh, int32_t size)
        {
            // view matrices
            glm::mat4 views[] = 
            {
            ...
            {
                glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_RGBA16F, size, size, 0, GL_RGB, GL_FLOAT, NULL);
            }
