...
namespace Nu {
	...
	#if UNIFORM_LOGGING
		#define NM_LOG_UNIFORM(...) NM_CORE_WARN(__VA_ARGS__)
	#else
		#define NM_LOG_UNIFORM
	#endif
	...
	void OpenGLShader::ReadShaderFromFile(const std::string& filepath)
	{
		...
		{
			NM_CORE_WARN("Could not read shader file {0}", filepath);
		}
	}
	...
	void OpenGLShader::CompileAndUploadShader()
	{
		...
		while (pos != std::string::npos)
		{
			...
			NM_CORE_ASSERT(eol != std::string::npos, "Syntax error");
			size_t begin = pos + typeTokenLength + 1;
			std::string type = m_ShaderSource.substr(begin, eol - begin);
			NM_CORE_ASSERT(type == "vertex" || type == "fragment" || type == "pixel", "Invalid shader type specified");
		...
		for (auto& kv : shaderSources)
		{
			...
			if (isCompiled == GL_FALSE)
			{
				...
				NM_CORE_ERROR("Shader compilation failed:\n{0}", &infoLog[0]);

				// We don't need the shader anymore.
				glDeleteShader(shaderRendererID);

				NM_CORE_ASSERT(false, "Failed");
			}
	...
	void OpenGLShader::UploadUniformInt(const std::string& name, int value)
	{
		...
		if (location != -1)
			...
			NM_LOG_UNIFORM("Uniform '{0}' not found!", name);
	}
