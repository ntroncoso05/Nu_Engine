...
namespace Nu {
		...
		: m_FilePath(path)
	{
		...
		NM_CORE_INFO("Loading texture {0}, srgb={1}", path, srgb);
		...
		NM_RENDER_S1(srgb, {
			...
			if (srgb)
			{
				...
				NM_CORE_INFO("Creating srgb texture width {0} mips", levels);
			...
			else
			{
				...
				glTexImage2D(GL_TEXTURE_2D, 0, NuToOpenGLTextureFormat(self->m_Format), self->m_Width, self->m_Height, 0, srgb ? GL_SRGB8 : NuToOpenGLTextureFormat(self->m_Format), GL_UNSIGNED_BYTE, self->m_ImageData);
				glGenerateMipmap(GL_TEXTURE_2D);
		...
		: m_FilePath(path)
	{
		...
		NM_CORE_ASSERT(faceWidth == faceHeight, "Non-square faces!");
		...
		NM_RENDER_S3(faces, faceWidth, faceHeight, {
			glGenTextures(1, &self->m_RendererID);
			...
			auto format = NuToOpenGLTextureFormat(self->m_Format);
