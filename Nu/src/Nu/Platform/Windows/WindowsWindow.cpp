...
namespace Nu {
	...
	void WindowsWindow::Init(const WindowProps& props)
	{
		...
		NM_CORE_INFO("Creating window {0} ({1}, {2})", props.Title, props.Width, props.Height);

		if (!s_GLFWInitialized)
		{
			// TODO: glfwTerminate on system shutdown
			int success = glfwInit();
			NM_CORE_ASSERT(success, "Could not intialize GLFW!");
		...
		NM_CORE_ASSERT(status, "Failed to initialize Glad!");
