...
namespace Nu {
	...
	struct LogStream : public Assimp::LogStream
	{
		...
		{
			NM_CORE_ERROR("Assimp error: {0}", message);
		}
	};
		...
		: m_FilePath(filename)
	{
		...
		NM_CORE_INFO("Loading mesh: {0}", filename.c_str());

		Assimp::Importer importer;

		const aiScene* scene = importer.ReadFile(filename, ImportFlags);
		if (!scene || !scene->HasMeshes())
			NM_CORE_ERROR("Failed to load mesh file: {0}", filename);

		aiMesh* mesh = scene->mMeshes[0];

		NM_CORE_ASSERT(mesh->HasPositions(), "Meshes require positions.");
		NM_CORE_ASSERT(mesh->HasNormals(), "Meshes require normals.");
		...
		{
			NM_CORE_ASSERT(mesh->mFaces[i].mNumIndices == 3, "Must have 3 indices.");
