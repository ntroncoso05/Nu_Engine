...
class EditorLayer : public Nu::Layer
{
public:
	EditorLayer()
	...
	{
		m_SimplePBRShader.reset(Nu::Shader::Create("assets/shaders/simplepbr.glsl"));
		m_QuadShader.reset(Nu::Shader::Create("assets/shaders/quad.glsl"));
		m_HDRShader.reset(Nu::Shader::Create("assets/shaders/hdr.glsl"));
		m_Mesh.reset(new Nu::Mesh("assets/meshes/cerberus.fbx"));
		m_SphereMesh.reset(new Nu::Mesh("assets/models/Sphere.fbx"));

		// Editor
		m_CheckerboardTex.reset(Nu::Texture2D::Create("assets/editor/Checkerboard.tga"));

		// Environment
		m_EnvironmentCubeMap.reset(Nu::TextureCube::Create("assets/textures/environments/Arches_E_PineTree_Radiance.tga"));
		m_EnvironmentIrradiance.reset(Nu::TextureCube::Create("assets/textures/environments/Arches_E_PineTree_Irradiance.tga"));
		m_BRDFLUT.reset(Nu::Texture2D::Create("assets/textures/BRDF_LUT.tga"));

		m_Framebuffer.reset(Nu::Framebuffer::Create(1280, 720, Nu::FramebufferFormat::RGBA16F));
		m_FinalPresentBuffer.reset(Nu::Framebuffer::Create(1280, 720, Nu::FramebufferFormat::RGBA8));
		...
		m_VertexBuffer.reset(Nu::VertexBuffer::Create());
		m_VertexBuffer->SetData(data, 4 * sizeof(QuadVertex));

		uint32_t* indices = new uint32_t[6] { 0, 1, 2, 2, 3, 0, };
		m_IndexBuffer.reset(Nu::IndexBuffer::Create());
	...
	virtual void OnUpdate() override
	{
		...
		Nu::UniformBufferDeclaration<sizeof(mat4), 1> quadShaderUB;
		...
		Nu::UniformBufferDeclaration<sizeof(mat4) * 2 + sizeof(vec3) * 4 + sizeof(float) * 8, 14> simplePbrShaderUB;
	...
	virtual void OnImGuiRender() override
	{
		...
		{
			auto& shaders = Nu::Shader::s_AllShaders;
		...
		ImGui::Separator();
		{
			...
			{
				std::string filename = Nu::Application::Get().OpenFile("");
				if (filename != "")
					m_Mesh.reset(new Nu::Mesh(filename));
			}
		}
		ImGui::Separator();
			...
			if (ImGui::CollapsingHeader("Albedo", nullptr, ImGuiTreeNodeFlags_DefaultOpen))
			{
				...
				if (ImGui::IsItemHovered())
				{
					...
					if (ImGui::IsItemClicked())
					{
						std::string filename = Nu::Application::Get().OpenFile("");
						if (filename != "")
							m_AlbedoInput.TextureMap.reset(Nu::Texture2D::Create(filename, m_AlbedoInput.SRGB));
					}
				}
				ImGui::SameLine();
				ImGui::BeginGroup();
				...
				{
					if (m_AlbedoInput.TextureMap)
						m_AlbedoInput.TextureMap.reset(Nu::Texture2D::Create(m_AlbedoInput.TextureMap->GetPath(), m_AlbedoInput.SRGB));
				}
				ImGui::EndGroup();
				ImGui::SameLine();
			...
			if (ImGui::CollapsingHeader("Normals", nullptr, ImGuiTreeNodeFlags_DefaultOpen))
			{
				...
				if (ImGui::IsItemHovered())
				{
					...
					if (ImGui::IsItemClicked())
					{
						std::string filename = Nu::Application::Get().OpenFile("");
						if (filename != "")
							m_NormalInput.TextureMap.reset(Nu::Texture2D::Create(filename));
					}
				}
				ImGui::SameLine();
			...
			if (ImGui::CollapsingHeader("Metalness", nullptr, ImGuiTreeNodeFlags_DefaultOpen))
			{
				...
				if (ImGui::IsItemHovered())
				{
					...
					if (ImGui::IsItemClicked())
					{
						std::string filename = Nu::Application::Get().OpenFile("");
						if (filename != "")
							m_MetalnessInput.TextureMap.reset(Nu::Texture2D::Create(filename));
					}
				}
				ImGui::SameLine();
			...
			if (ImGui::CollapsingHeader("Roughness", nullptr, ImGuiTreeNodeFlags_DefaultOpen))
			{
				...
				if (ImGui::IsItemHovered())
				{
					...
					if (ImGui::IsItemClicked())
					{
						std::string filename = Nu::Application::Get().OpenFile("");
						if (filename != "")
							m_RoughnessInput.TextureMap.reset(Nu::Texture2D::Create(filename));
					}
				}
				ImGui::SameLine();
		...
		if (ImGui::BeginMenuBar())
		{
			if (ImGui::BeginMenu("Docking"))
			{
				...
				if (ImGui::MenuItem("Flag: PassthruCentralNode", "", (opt_flags & ImGuiDockNodeFlags_PassthruCentralNode) != 0))       opt_flags ^= ImGuiDockNodeFlags_PassthruCentralNode;
	...
	virtual void OnEvent(Nu::Event& event) override
	{
	}
	private:
		std::unique_ptr<Nu::Shader> m_Shader;
		std::unique_ptr<Nu::Shader> m_PBRShader;
		std::unique_ptr<Nu::Shader> m_SimplePBRShader;
		std::unique_ptr<Nu::Shader> m_QuadShader;
		std::unique_ptr<Nu::Shader> m_HDRShader;
		std::unique_ptr<Nu::Mesh> m_Mesh;
		std::unique_ptr<Nu::Mesh> m_SphereMesh;
		std::unique_ptr<Nu::Texture2D> m_BRDFLUT;

		struct AlbedoInput
		{
			glm::vec3 Color = { 0.972f, 0.96f, 0.915f }; // Silver, from https://docs.unrealengine.com/en-us/Engine/Rendering/Materials/PhysicallyBased
			std::unique_ptr<Nu::Texture2D> TextureMap;
			bool SRGB = true;
		...
		std::unique_ptr<Nu::Framebuffer> m_Framebuffer, m_FinalPresentBuffer;

		std::unique_ptr<Nu::VertexBuffer> m_VertexBuffer;
		std::unique_ptr<Nu::IndexBuffer> m_IndexBuffer;
		std::unique_ptr<Nu::TextureCube> m_EnvironmentCubeMap, m_EnvironmentIrradiance;
		...
		// Editor resources
		std::unique_ptr<Nu::Texture2D> m_CheckerboardTex;
};
