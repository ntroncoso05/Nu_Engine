...
{     
  ...
  vertex.Position = (transform * vec4(a_position, 1.0f)).xyz;
  gl_Position = u_proj * u_view * transform * vec4(a_position, 1.0f);
...
// constants
const vec3 BLOOM_THRESHOLD = vec3(0.2126f, 0.7152f, 0.0722f);
...
vec3 ComputeAmbientLight(vec3 N, vec3 V, vec3 F0, vec3 albedo, float roughness, float metallic) 
{
 // angle between surface normal and light direction.
  ...
  // sample pre-filtered map at correct mpmap level.
  int mipLevels = 5;
  vec3 Lr = 2.0f * cosTheta * N - V;
...
vec3 ComputeDirectLights(vec3 N, vec3 V, vec3 F0, vec3 albedo, float roughness, float metallic) 
{
  ...
  for (int i = 0; i < u_nbrDirectLight; ++i) 
  {
    ...
    vec3 FS = FresnelSchlick(clamp(dot(H, V), 0.0f, 1.0f), F0);
    float GS = GeometrySmithGGX(NdotV, NdotL, roughness);      

    // diffuse light
    vec3 diffuse = (vec3(1.0f) - FS) * (1.0f - metallic);

    // specular light
    vec3 specular = (NDF * GS * FS) / max(4.0f * NdotV * NdotL, 0.0001f); 
...
vec3 ComputeSpotLights(vec3 N, vec3 V, vec3 F0, vec3 albedo, float roughness, float metallic) 
{
  ...
  for (int i = 0; i < u_nbrSpotLight; ++i) 
  {
    ...
    float spotFactor = clamp((theta - u_spotLights[i].CutOff) / epsilon, 0.0f, 1.0f);
...
float ComputeShadow()
{
  // compute lightspace fragment position
  vec4 position = u_lightSpace * vec4(vertex.Position, 1.0f);
  // normalize projection coordinates 
  vec3 coords = (position.xyz / position.w) * 0.5f + 0.5f;

  // pixel size (1024 -> map size)
  float pixelSize = 1.0f / 1024;
  float shadow = 0.0f;
  float bias = 0.005f;
  ...
  for(int x = -1; x <= 1; ++x)
  {
    for(int y = -1; y <= 1; ++y)
    {
      ...
      shadow += (position.z - bias) > depth ? 1.0f : 0.0f;
    }
  }
...
void main()
{
  ...
  { 
    ...
    N = 2.0f * texture(u_material.NormalMap, vertex.UVs).rgb - 1.0f;
  ...
  vec3 result = ComputeAmbientLight(N, V, F0, albedo, roughness, metallic);
    ...
    result *= (1.0f - ComputeShadow());

    // output brightness
    if(dot(result, BLOOM_THRESHOLD) > 1.0f)
    {
      out_brightness = vec4(result, 1.0f);
    }
    else
    {
